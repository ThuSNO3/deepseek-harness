...
| `ctx.uiAutomation` | `seam` | [`ui-automation`](../packages/interaction/ui-automation) | - | [`tool-ui-automation`](../packages/interaction/tool-ui-automation) | - | Host Providers own paged semantic observations and delivery; the Consumer enforces one pending observation and one accepted action per Agent. |
