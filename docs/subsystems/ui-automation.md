...
The [dsh-ui-automation package](../../packages/interaction/ui-automation/README.md) defines semantic Host UI observation and bounded actions. Product Providers own target identity, safety, approval, transport, input delivery, and condition semantics. The [tool Consumer](../../packages/interaction/tool-ui-automation/README.md) owns model schemas and sequencing.

Source: [packages/interaction/ui-automation/src/index.ts](../../packages/interaction/ui-automation/src/index.ts)

## Semantic UI data

A null cursor starts an observation lease; Provider-minted cursors continue it. Every page shares one snapshot id and surface revision. Refs remain opaque and valid only in that lease. An action receipt reports admission, while a wait reports the later observed condition. Fill values distinguish model-visible literal text from an opaque Host slot.

Source: [types.ts](../../packages/interaction/ui-automation/src/types.ts)
